...
class Crawler(object):
    ...
    def save(self, dict_res={}):
        for u,v in dict_res.iteritems():
            redis_one.hset(self.sitemap_prefix, u,  YhCompress.compress(v))
        
    def notsaved(self, list_url=[]):
        if list_url:
            list_res = redis_one.hmget(self.sitemap_prefix, list_url)
    ...
    def process(self, start_url='http://www.120ask.com/list/all', start=0, end=2538777):
        try:
            ...
            while i <= end:
                ...
                self.save(dict_res)
                i += 100
                logger.error('Crawler %s %s %s' % (list_url[0], len(dict_res), redis_one.hlen(self.sitemap_prefix)))
        except:
